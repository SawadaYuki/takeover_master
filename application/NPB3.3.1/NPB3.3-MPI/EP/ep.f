...
c      print "('embar:first allreduce')"
      call mpi_allreduce(sx, x, 1, dp_type,
     >                   MPI_SUM, MPI_COMM_WORLD, ierr)
      sx = x(1)
c      print "('embar:second allreduce')"
      call mpi_allreduce(sy, x, 1, dp_type,
     >                   MPI_SUM, MPI_COMM_WORLD, ierr)
      sy = x(1)
c      print "('embar:third allreduce')"
 ...
 11      format ('EP Benchmark Results:'//'CPU Time =',f10.4/'N = 2^',
     ...
     >                      cs2, cs3, cs4, cs5, cs6, cs7)
      ...
      call MPI_allreduce(t1m, tsum,  t_last+2, dp_type, MPI_SUM, 
     >                MPI_COMM_WORLD, ierr)
      call MPI_allreduce(t1m, tming, t_last+2, dp_type, MPI_MIN, 
     >                MPI_COMM_WORLD, ierr)
      call MPI_allreduce(t1m, tmaxg, t_last+2, dp_type, MPI_MAX, 
     >                MPI_COMM_WORLD, ierr)

c -- sawada org:s 
c$$$     call MPI_allreduce(t1m, tsum,  t_last+2, dp_type, MPI_SUM, 
c$$$     >                MPI_COMM_WORLD, ierr)
c$$$      call MPI_allreduce(t1m, tming, t_last+2, dp_type, MPI_MIN, 
c$$$     >                MPI_COMM_WORLD, ierr)
c$$$      call MPI_allreduce(t1m, tmaxg, t_last+2, dp_type, MPI_MAX, 
c$$$     >                MPI_COMM_WORLD, ierr)
c -- sawada org:e 
      if (node .eq. 0) then
         write(*, 800) no_nodes
