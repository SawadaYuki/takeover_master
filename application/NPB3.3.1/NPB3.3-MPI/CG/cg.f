...
c -- sawada org:s 
      call mpi_allreduce(t,tmax,1,dp_type,MPI_MAX,mpi_comm_world,ierr)
c -- sawada org:e 
      if( me .eq. root )then
         write(*,100)
...
c -- sawada org:s 
      call MPI_allreduce(t1, tsum,  t_last+2, dp_type, MPI_SUM, 
     >                MPI_COMM_WORLD, ierr)

      call MPI_allreduce(t1, tming, t_last+2, dp_type, MPI_MIN, 
     >                MPI_COMM_WORLD, ierr)

      call MPI_allreduce(t1, tmaxg, t_last+2, dp_type, MPI_MAX, 
     >                MPI_COMM_WORLD, ierr)
c -- sawada org:e 
...
c -- sawada org:s 
      if( me .eq. root )then
         print "('DOa dmtcp_command')"
         call system('dmtcp_command -k')
      endif
c -- sawada org:e 
...
c---------------------------------------------------------------------
     ...
     >                      request,
     >                      ierr )
c-------- Sent Message Size is big:count = 7600 2番目のMPI_send
     ...
     >                      mpi_comm_world,
     >                      ierr )
c            print "('conj_grad:No.2 mpi_send')"
c            print "('callsite a little Send')"
            call mpi_wait( request, status, ierr )
            if (timeron) call timer_stop(t_rcomm)
...
c---------------------------------------------------------------------
     ...
     >                      request,
     >                      ierr )
c----- Sent Message Size is big:count = 7600 3番目のMPI_send
     ...
     >                      mpi_comm_world,
     >                      ierr )
c            print "('conj_grad:No.3 mpi_send')"
c            print "('callsite big Send')"
            call mpi_wait( request, status, ierr )
c            print "('callsite a little Wait')"
            if (timeron) call timer_stop(t_rcomm)
         else
...
c---------------------------------------------------------------------
     ...
     >                      mpi_comm_world,
     >                      ierr )
c------------ APP% = 14% MPI% = 45% 4番目のMPI_Wait
            call mpi_wait( request, status, ierr )
c            print "('callsite big Wait')"
            if (timeron) call timer_stop(t_rcomm)
