...
void job_master(int rank, int n, 
  ...
  for(i=1; i<jobs + pnos; i++){//job数分+workerプロセス数分
    ...
    int x_max =  eachjobs*status.MPI_SOURCE;
    int x_min;
    if(status.MPI_SOURCE == 1) {
      x_min = 1;
    }else{
      x_min =  eachjobs*(status.MPI_SOURCE-1)+1;
    }
#ifdef DEBUG
...
#ifdef DEBUG
      printf("recv from %d:recv.jno = %d,array[%d]=%d\n",status.MPI_SOURCE,recv.jno,(status.MPI_SOURCE)-1);
#endif
