...
namespace jalib
{
  ...
  public:
      ...
      int port() const { struct sockaddr_in addr;
			 ...
			 if (-1 == getsockname(_sockfd,
			   ...
			   JTRACE("getsockname == -1"); // sawada 10/15
			   return -1;
			 }
			 else{
			   JTRACE(" getscokname success")(_sockfd)(ntohs(addr.sin_port));
			   return (int)ntohs(addr.sin_port); // sawada 10/15
			 }
		       }
  ...
  class JChunkReader : public JReaderInterface
  {
    public:
      ...
      bool hadError() const { 
	//JTRACE("check error flag")( _hadError)(!_sock.isValid())(_sock.isValid());
  ...
  class JMultiSocketProgram
  {
    ...
    protected:
      //JReaderInterface*型の要素のための可変長配列(_dataSockets:クラスはjalib)
