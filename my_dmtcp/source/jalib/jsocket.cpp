                              ...
                              int port /* == -1*/ )
{
  ...
  if ( hostname == NULL ) {
    ...
    if (port != -1){
      ...
      JTRACE("check: port at JScokAddr")(ntohs(_addr[0].sin_port))(htons(port));
    }
    return;
  }
  ...
  JTRACE("check:port")(ntohs(_addr.sin_port));
#else
  ...
  if (e == 0) {
	...
	JTRACE("check port:array for storing all nesessary addresses")(ntohs(_addr[i].sin_port))(i);
      }
    }

  } else { // else (hostname, port) not valid; poison the port number
    _addr[0].sin_port = (unsigned short)-2;
    JTRACE("check port")(ntohs(_addr[0].sin_port));
  }
...
bool jalib::JSocket::connect ( const JSockAddr& addr, int port )
{
  ...
  for (unsigned int i=0; i< addr._count; i++) {
    ret = JSocket::connect((sockaddr*)(addr._addr + i),
                           sizeof(addr._addr[0]),
                           port);
    JTRACE("After connect,check: port")(port);
...
bool jalib::JSocket::bind ( const JSockAddr& addr, int port )
{
  ...
  for( unsigned int i=0; i<addr._count; i++){
    ...
    JTRACE("check: port")(ntohs(addrbuf.sin_port));
...
jalib::JSocket jalib::JSocket::accept ( struct sockaddr_storage* remoteAddr,socklen_t* remoteLen )
{
  if ( remoteAddr == NULL || remoteLen == NULL ){
    JTRACE("remoteAddr == NULL || remoteLen == NULL");
    return JSocket ( jalib::accept ( _sockfd,NULL,NULL ) );
  }
  else{//remoteAddrは接続してきたclientのアドレス構造体のポインタ
...
void jalib::JSocket::enablePortReuse()
{
  ...
  JTRACE("called setsockopt in enablePortReuse()");
  if (jalib::setsockopt(_sockfd, SOL_SOCKET, 
...
ssize_t jalib::JSocket::readAll ( char* buf, size_t len )
{
  char temp[len+1];
  strcpy(temp,buf);
  //  JTRACE("JSocket::readAll,*buf is firstchr,temp is allchr")(temp)(len);
...
{
  JTRACE("check isValid()")(_sockfd >= 0)(_sockfd);
  return _sockfd >= 0;
}
...
bool jalib::JChunkReader::readOnce()
{
  if ( !ready() )
  {
    ...
    if ( cnt <= 0 && errno != EAGAIN && errno != EINTR )
      _hadError = true;
    //JTRACE("_hadError = true")(_sock.sockfd())(_hadError);
    if ( cnt > 0 )
      _read += cnt;
  }
  return _read > 0;
...
{
  //JTRACE("check error")(_hadError)(_sock.isValid())(!_sock.isValid());
...
jalib::JChunkReader::JChunkReader ( JSocket sock, int chunkSize )
    ...
    , _hadError ( false )
{
  memset ( _buffer,0,_length ); //中身見ても分らない
}
...
{
  _writes.push_back ( write );//ベクタの末尾に要素を追加する,capacity=8
}
...
void jalib::JMultiSocketProgram::monitorSockets ( double dblTimeout )
{
  ...
  */
  ...
  for ( ;; )
  {
    ...
    JTRACE("collect data fds in rfds");
    //collect data fds in rfds, cleanup dead sockets
    for ( i=0; i<_dataSockets.size(); ++i )
    {
      //JTRACE("_dataSockets[i]->hadError()")(_dataSockets[i]->hadError())( _dataSockets[i]->socket().sockfd());
      if ( !_dataSockets[i]->hadError() ) // <--0だとまずい...状況:_hadError = 0 && _sock.isValid() = 1
      {
	JTRACE(" Eregular ")( _dataSockets[i]->socket().sockfd());
      ...
      else
      {
        ...
        //socket is dead... remove it
        JTRACE ( "disconnect" ) ( i ) ( _dataSockets[i]->socket().sockfd() )(_dataSockets.size());
    ...
    JTRACE("collect all writes");
    //collect all writes in wfds, cleanup finished/dead
    for ( i=0; i<_writes.size(); ++i )
    {
      //JTRACE("check")(_writes[i]->hadError())(_writes[i]->isDone())(closedFds.find(_writes[i]->socket().sockfd())==closedFds.end());
    ...
    else if ( retval > 0 )
    {
      ...
      for ( i=0; i<_writes.size(); ++i )
      {
        ...
        if ( fd >= 0 && FD_ISSET ( fd, &wfds ) )
        {
          ...
          _writes[i]->writeOnce();
        }else{//sawada oroginal
	  JTRACE("sawada org: Not writing data")(fd)(FD_ISSET ( fd, &wfds ));
	}
      }
	  ...
	  JTRACE("receiving data")(i)(_dataSockets[i]->socket().sockfd());
	    ...
	    JTRACE("sawada org: Not doing onData()")(_dataSockets[i]->readOnce());
	  }
        }else{//sawada org
	  JTRACE("sawada org: Not receiving data")(fd);
	}
      }
      ...
      for ( i=0; i<_listenSockets.size(); ++i )
      {
        ...
        if ( fd >= 0 && FD_ISSET ( fd, &rfds ) )
        {
          ...
          if ( sk.isValid() )
          {
	    JTRACE("next is onConnect")(i)(_listenSockets[i].sockfd());
    ...
    if ( timeoutEnabled )
    {
      ...
      else
      {
        ...
        stoptime = tmptime;
        timeradd ( &timeoutInterval,&stoptime,&stoptime );
                 JTRACE("timeout interval")(timeout);
        onTimeoutInterval();
      }
    }
  }
