...
#define SIGCANCEL (__SIGRTMIN) /* aka SIGTIMER */
  ...
  /* Release user thread after we've initialized. */ //not created ckpt file or dir
  sem_post(&sem_start);
  ...
  while (1) {
    ...
    restoreInProgress = 0;
    suspendThreads(); //→ sem_wait(&semNotifyCkptThread);
    ...
    //orterunは中身は空だが、ckpt fileやdirができている
    //※orterun以外のプロセスのckpt fileが作成されていて、中身には既に書きこみがされている
    MtcpHeader mtcpHdr;
...
void stopthisthread (int signum)
{
  ...
  DPRINTF("after sigsetjmp/getcontext. tid: %d, vtid: %d, saved_sp: %p\n",
    ...
    if (!restoreInProgress) {
      ...
      sem_post(&semNotifyCkptThread);                                     //not create ckpt file
      ...
      // this is systemcall  sem が指すセマフォの値を 1 減らす (ロックする)
      /*
