...
{
  //JTRACE("called dmtcp::Util::writeAll");//恐らく表示回数が膨大なせいで、makeは通るがプログラムが起動しない
...
{
  if (oldfd != newfd) {
    JTRACE("odldfd!=newfd")(oldfd)(newfd);
...
{
  changeFd(oldfd, newfds[0]);// -> real_dup2(oldfd,newfd[0]);
  //JTRACE("size")(newfds.size());
  for (size_t i = 1; i < newfds.size(); i++) {
    JTRACE("dupFds")(oldfd)(newfds[i])(newfds.size())(newfds[0]);
...
// std::vector<std::string> dmtcp::Util::split(const std::string &str, char sep)
// {
...
// std::vector<int> dmtcp::Util::split_int(const std::string &str, char sep)
// {
//     std::vector<int> v;
//     std::stringstream ss(str);
//     std::string buffer;
