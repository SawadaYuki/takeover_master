...
void dmtcp::SocketConnList::drain()
{
  ...
  JTRACE(" dmtcp::SocketConnList::drain() is called,next is monitorSockets()");
  ...
  for (it = discn.begin(); it != discn.end(); it++) {
    ...
    JTRACE("Before  TcpConnection *con =")(id);
    TcpConnection *con =
...
void dmtcp::SocketConnList::preCkpt()
{
  ...
  JTRACE("now receive ... doRecvHandshakes start");
  //now receive
  for (iterator i = begin(); i != end(); ++i) {
    Connection *con = i->second;
    if (con->hasLock() && con->conType() == Connection::TCP) {
...
void dmtcp::SocketConnList::postRestart()
{
  ConnectionRewirer::instance().openRestoreSocket(_hasIPv4Sock, _hasIPv6Sock,
                                                  _hasUNIXSock);
  JTRACE("next,Connectionlist::postRestart");
...
{
  if (isRestart) {
    JTRACE("called dmtcp::SocketConnList::registerNSData( bool );Next => ConnectionRewirer::instance().registerNSData()");
    ConnectionRewirer::instance().registerNSData();
  }
  ConnectionList::registerNSData(isRestart);//ConnectionList::registerNSData(bool)は仮想関数 実装{}
  JTRACE("After ConnectionList::registerNSData(isRestart)");
}
...
void dmtcp::SocketConnList::refill(bool isRestart)
{
  ...
  JTRACE("After instance().refillAllSockets(),next is ConnectionList::refill(isRestart)");
  ...
  for (size_t i = 0; i < fds.size(); ++i) {
    ...
    if (device == jalib::Filesystem::GetControllingTerm()) {
      ...
      JNOTE("found pre-existing socket... will not be restored")
        (fd) (device);
      JTRACE("Before TcpConnection* con = new TcpConnection(0, 0, 0);");
      TcpConnection* con = new TcpConnection(0, 0, 0);
      con->markPreExisting();
      JTRACE("Before add(fd, con)")(fd)(con);
      add(fd, con);
    }
  }
}
...
dmtcp::Connection *dmtcp::SocketConnList::createDummyConnection(int type)
{
  if (type == Connection::TCP) {
    JTRACE("called *dmtcp::SocketConnList::createDummyConnection:type = TCP");
    return new TcpConnection();
  } else if (type == Connection::RAW){
    JTRACE("called *dmtcp::SocketConnList::createDummyConnection:type = RAW");
    return new RawSocketConnection();
  }
  JTRACE("called *dmtcp::SocketConnList::createDummyConnection:return NULL");
  return NULL;
}
