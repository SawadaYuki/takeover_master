...
{
  JTRACE("called void dmtcp::DmtcpWorker::resetOnFork()");
                                               ...
                                               DmtcpMessageType type)
{
  /*
  JTRACE("test")(msgStr);
  if(msgStr == "REGISTER_NAME_SERVICE_DATA"){
    return;
  }
  */
  ...
  if (type == DMT_DO_SUSPEND) {
    ...
    JTRACE("During Sleep\n");
    sleep(30);
    JTRACE("Release Sleep,Next sendMsg\n");
    #endif

    JTRACE("Before sendMsgToCoordinator(msg)");
    CoordinatorAPI::instance().sendMsgToCoordinator(msg);
    JTRACE("After sendMsgToCoordinator(msg)");
    /*
    JTRACE("After SendMsg,During Sleep\n");
    sleep(5);
    JTRACE("Release Sleep\n");
    */
  }

  JTRACE("waiting for " + msgStr + " message, next instance().recvMsgFromCoordinator(&msg)");
  #if 0
  JTRACE("During Sleep\n");
  sleep(30);
  JTRACE("Release Sleep,Next RecvMsg\n");
  #endif
 
 
  JTRACE("Before recvMsgFromCoordinator(&msg)")(msg.type);
  
  
  CoordinatorAPI::instance().recvMsgFromCoordinator(&msg);
  JTRACE("After recvMsgFromCoordinator(&msg)")(msg.type)(msg.extraBytes)(msg.numPeers);
  ...
  JTRACE("before msg.assertValid()");
  msg.assertValid();
  JTRACE("After msg.assertValid()");
...
void dmtcp::DmtcpWorker::waitForStage3Refill(bool isRestart)
{
  ...
  eventHook(DMTCP_EVENT_REGISTER_NAME_SERVICE_DATA, &edata);//dmtcp_event_hook = NULL
...
{
  JTRACE("called dmtcp::DmtcpWorker::eventHook()");
  ...
  if (dmtcp_event_hook != NULL) {
    JTRACE("next,dmtcp_event_hook")(event)(DMTCP_EVENT_POST_EXEC)(DMTCP_EVENT_RESUME_USER_THREAD);
