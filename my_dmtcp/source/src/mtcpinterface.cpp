...
//sawada
bool myIsRestart = false;

using namespace dmtcp;
char procname[100];//sawada 2015_8_4
...
{
  if (isRestart) {
    JTRACE("second prctlRestoreProcessName()");
    prctlRestoreProcessName();
  }
  ...
  JTRACE("Before syscall(DMTCP_FAKE_SYSCALL)");
  syscall(DMTCP_FAKE_SYSCALL);
  JTRACE("After syscall(DMTCP_FAKE_SYSCALL)");
}
...
void prctlRestoreProcessName()
{
  ...
  // which is supported on >= 2.6.11
    ...
    if (prctl(PR_SET_NAME, prctlPrgName) != -1) {
      strcpy(procname,prctlPrgName);//sawada 2015_8_4
