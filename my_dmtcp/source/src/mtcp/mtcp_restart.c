...
int main(int argc, char *argv[], char **environ)
{
  ...
  int mtcp_sys_errno;
  int simulate = 0;
  MTCP_PRINTF("MTCP_PRINTF test:argc = %d\n",argc);
  if (argc == 1) {
...
{int x=1; while(x);}
  ...
  rinfo.text_offset = -1;
  shift;
  //MTCP_PRINTF("Before while(argc~ :argc = %d\n",argc); // 4 or 5
  while (argc > 0) {
  ...
  if (rinfo.fd != -1) {
    int mtcpHdr_size = sizeof mtcpHdr;//sawada
    MTCP_PRINTF("next:mtcp_readfile(rinfo.fd= %d,mtcpHdr_size = %d\n",rinfo.fd,mtcpHdr_size);
    mtcp_readfile(rinfo.fd, &mtcpHdr, sizeof mtcpHdr);
  } else {
    MTCP_PRINTF("open ckpt image:%s\n",ckptImage);
