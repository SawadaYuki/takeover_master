...
//sawad org :start
#include <sstream>
std::vector<dmtcp::UniquePid> parentUpid;
map<dmtcp::string,int> eachNode;
std::vector<uint64_t> HostIdofConnect;
map<dmtcp::UniquePid,int> sentHostidForParent;

//restart時のDMT_DO_REGISTER_NAME_SERVICE_DATAにプロセスの所在情報関連の処理を行うためのフラグ
static dmtcp::WorkerState stateFlag;

//writeAll(const char*,size_t)のためint型ではない
static dmtcp::string pidSet = "";
static dmtcp::string ipSet = "";
static dmtcp::string isParent = "";
  ...
  : _sock(sock)
{
  ...
  // sawada org :start
  //coordinatorと通信するためのportで、openedRestoreSocket()で得られるportではない
  unsigned short port = htons(in->sin_port);
  stateFlag =  hello_remote.state;
  JTRACE("called CoordClient()")(_realPid)(_clientNumber)(_identity)(_identity.hostid())(_ip)(port)(_state)(stateFlag);
  //sawada org :end
}

// void dmtcp::CoordClient::readProcessInfo(dmtcp::DmtcpMessage& msg) <- default
void dmtcp::CoordClient::readProcessInfo(dmtcp::DmtcpMessage& msg,dmtcp::CoordClient *client)
{
  ...
  if (msg.extraBytes > 0) {
    ...
    //hostid()=uint64_t型：64bit=8byte でかいな
    //in_addr型は32bit=4byte
    ...
    JTRACE("readProcessInfo top")(_hostname)(_progname)(client->ip())(client->identity())(client->identity().pid())(client->identity().hostid())(client->realPid())(client->virtualPid())(sizeof(client->ip()));
  
    if(eachNode.empty()){//host nodeがまず記録
      eachNode[client->ip()]=1;
      parentUpid.push_back(client->identity());
...
void dmtcp::DmtcpCoordinator::updateMinimumState(dmtcp::WorkerState oldState)
{
...
#endif
  {
    ...
    JNOTE ( "restarting all nodes" )(TimePreparation);
    broadcastMessage ( DMT_DO_RESUME );
    // sawada org:s 
    gettimeofday(&start, NULL); 
    JTRACE("start calc in coordinator")((start.tv_sec)*1000+(start.tv_usec)/1000);
    // sawada org:e 
...
void dmtcp::DmtcpCoordinator::onData(CoordClient *client)
{
  ...
  JASSERT(client != NULL);
  client->sock() >> msg;
  JTRACE("After client->sock()>>msg")(inet_ntoa(msg.ipAddr));
  msg.assertValid();//msg check(magicBits and sie is mismatch?)
  char *extraData = 0;
  JTRACE("After msg.assertValid()")(msg.extraBytes);
  if (msg.extraBytes > 0) {
    
    extraData = new char[msg.extraBytes];
    client->sock().readAll(extraData, msg.extraBytes);
    JTRACE("msg.extraBytes > 0")(msg.from)(*extraData)(msg.extraBytes);

  }

  switch ( msg.type )
  {
    case DMT_OK:// change state
    {
      ...
      updateMinimumState(oldState);//On each state,do broadcastMesseage
      break;
    }
    case DMT_UNIQUE_CKPT_FILENAME:
      uniqueCkptFilenames = true;
      // Fall though
    case DMT_CKPT_FILENAME:// On checkpointing
    {
      JASSERT ( extraData!=0 )
    ...
    case DMT_REGISTER_NAME_SERVICE_DATA:
      { // 全プロセスの受信が終るまで、他のプロセスは何回もメッセージを送る
      JTRACE ("received REGISTER_NAME_SERVICE_DATA msg") (client->identity());
      JTRACE("registerData")(*extraData)(msg._magicBits)(msg._msgSize)(msg.extraBytes)(msg.type)(msg.from)(msg.compGroup)(msg.virtualPid)(msg.realPid)(msg.nsid)(msg.keyLen)(msg.numPeers)(msg.isRunning)(msg.coordCmd)(msg.coordCmdStatus)(msg.coordTimeStamp)(msg.theCheckpointInterval)(inet_ntoa(msg.ipAddr));
    ...
    case DMT_NAME_SERVICE_QUERY:
    {
      ...
      JTRACE("respondToQuery")(&client->sock())(*extraData)(msg._magicBits)(msg._msgSize)(msg.extraBytes)(msg.type)(msg.from)(msg.from.pid())(msg.compGroup)(msg.virtualPid)(msg.realPid)(msg.nsid)(msg.keyLen)(msg.numPeers)(msg.isRunning)(msg.coordCmd)(msg.coordCmdStatus)(msg.coordTimeStamp)(msg.theCheckpointInterval)(inet_ntoa(msg.ipAddr));
      //この段階のmsg.ipAddr.s_addr=0
    ...
    case DMT_NULL:
      JTRACE("msg.type= DMT_NULL:next call onDisconnect(client)");
...
void dmtcp::DmtcpCoordinator::onDisconnect(CoordClient *client)
{
  ...
  ComputationStatus s = getStatus();
  JNOTE("check")(s.numPeers);
  if (s.numPeers < 1) {
    if (exitOnLast) {
      JNOTE ("last client exited, shutting down..");
      handleUserCommand('q');
    } else {
      JNOTE("next,call removeStaleSharedAreaFile()");
      removeStaleSharedAreaFile();
    }
    ...
    //JNOTE("next,call updateMinimumState(client->state())");
    updateMinimumState(client->state());
  }
  //JNOTE("onDisconnect is end");
}
...
void dmtcp::DmtcpCoordinator::onConnect()
{
  ...
  // original start
  sockaddr_in *sn = (sockaddr_in*) &remoteAddr;
  unsigned short port = htons(sn->sin_port);
  char *ip = inet_ntoa(sn->sin_addr);
  //end
    
  JTRACE("Start onConnect(): accepting new connection")(ip)(port)(remote.sockfd()) (JASSERT_ERRNO);

  if (!remote.isValid()) {
    JTRACE("if(!remote.isValid())-->remote.close");
    remote.close();
    return;
  }
  ...
  // DmtcpMessageはメンバ変数に,struct in_addr ipAddrを含む
  // in_addr構造体は、in_addr_tのs_addrしかメンバに持たない構造体
  //hello_remoteをremoteへ読み込む
  remote >> hello_remote;
  if (!remote.isValid()) {
    JTRACE("if(!remote.isValid())-->remote.close");
    remote.close();
  ...
  if (hello_remote.type == DMT_NAME_SERVICE_WORKER) {
    // registerNSData() ->  dmtcp_send_key_val_pair_to_coordinator()
    JTRACE("hello_remote.type==DMT_NAME_SERVICE_WORKER");
   
    //この段階で起動するnodeの正しいIPアドレスが振られる
    CoordClient *client = new CoordClient(remote, &remoteAddr, remoteLen,
		                          hello_remote);
    /*  if(client._progname == "orterun"){
	}*/
  ...
  if (hello_remote.type == DMT_NAME_SERVICE_QUERY) {
    ...
    remote.readAll(extraData, hello_remote.extraBytes);//<-extraData = keyのはず

    JTRACE ("received NAME_SERVICE_QUERY msg on running") (extraData)(hello_remote.from);
  ...
  CoordClient *client = new CoordClient(remote, &remoteAddr, remoteLen,
                                        hello_remote);
  JTRACE("Second [new CoordClient]")(client->ip());
  //sawada org:s 
  ...
  if( hello_remote.extraBytes > 0 ){
    client->readProcessInfo(hello_remote,client);
  }
  ...
  if (hello_remote.type == DMT_RESTART_WORKER) {
    ...
    client->virtualPid(hello_remote.from.pid()); // vitrualPidを割り当てる checkpoint時の
  ...
  updateCheckpointInterval(hello_remote.theCheckpointInterval);
  JNOTE ( "worker connected" ) ( hello_remote.from );// parent ,MPI process,,,,,

  clients.push_back(client);// incliment client'size
  addDataSocket(client);

  JTRACE("END") (clients.size()); // size=1
}
          ...
          const struct sockaddr_storage* remoteAddr, socklen_t remoteLen)
{
  ...
  unsigned short port = htons(sin->sin_port);//original
  JTRACE("validateRestartingWorkerProcess : check ip and port")(remoteIP)(port);
  ...
  if (compId == dmtcp::UniquePid(0,0,0)) {
    ...
    JNOTE ( "FIRST dmtcp_restart connection.  Set numPeers. Generate timestamp" )
      ( numPeers ) ( curTimeStamp ) ( compId );// numpeers=restart processes
   ...
   const struct sockaddr_storage* remoteAddr, socklen_t remoteLen)
{
  ...
  unsigned short port = htons(sin->sin_port);//original
  JTRACE("validateNewWorkerProcess: check ip and port")(remoteIP)(port);
...
bool dmtcp::DmtcpCoordinator::startCheckpoint()
{
       ...
       && !workersRunningAndSuspendMsgSent )
  {
    ...
    JTRACE("Before if (s.numPeers > 0)")(s.minimumState)(s.minimumStateUnanimous)(workersRunningAndSuspendMsgSent)(s.numPeers);
    if (s.numPeers > 0) {
                                               ...
                                               int numPeers)
{
  ...
  JTRACE("check msg")(msg.type)( msg.compGroup)(numPeers)(clients.size());
  ...
  JTRACE("check flag value")(msg.type)(stateFlag)(isChngIPCDomain);
  for (size_t i = 0; i < clients.size(); i++) { // number of client,send message
    ...
    clients[i]->sock() << msg;
    JTRACE("After send msg to client")(i)(clients[i])(msg.state);
...
static void calcLocalAddr()
{
  ...
  if (error == 0) {
    ...
    for (res = result; res != NULL; res = res->ai_next) {
      ...
      /* sawada ori:start */
       unsigned short port = htons(s->sin_port);
       char *ip = inet_ntoa(s->sin_addr);
       JTRACE("check ip and port")(ip)(port);
       /* sawada ori :end */
...
void dmtcp::DmtcpCoordinator::eventLoop(bool daemon)
{
  ...
  while (true) {
    ...
    //timeout=timeout+10000;//sawada 9/17
    //epoll ファイルディスクリプタ epollFd のイベントを最大 timeout ミリ秒待つ
    //要求された I/O に対して準備ができているファイルディスクリプタの数を返す
    ...
    //EPOLLERR...関連付けられたファイルディスクリプターにエラー条件が起こった
    //EPOLLHUP...関連付けられたファイルディスクリプターにハングアップが起こった
    //EPOLLRDHUP...ストリームソケットの他端が、コネクションの close 、 またはコネクションの書き込み側の shutdown を行った
    for (int n = 0; n < nfds; ++n) {
      JNOTE("number of event")(n)(nfds);
      ...
      if ((events[n].events & EPOLLHUP) ||
        ...
        if (ptr == (void*) STDIN_FILENO) {//STDIN_FILENO=標準入力のファイル記述子0
          JASSERT(epoll_ctl(epollFd, EPOLL_CTL_DEL, STDIN_FILENO, &ev) != -1)
            (JASSERT_ERRNO);
          close(STDIN_FD);
        } else {
          JTRACE("next,call onDisconnect _sawada")(n);
          onDisconnect((CoordClient*)ptr);
        }
      } else if (events[n].events & EPOLLIN) {
        //EPOLLIN...関連付けられたファイルに対して、 read操作が可能である
        if (ptr == (void*) listenSock) {
          JNOTE("ptr==listenSock");
          onConnect();
        } else if (ptr == (void*) STDIN_FILENO) {
          char buf[1];
          int ret = Util::readAll(STDIN_FD, buf, sizeof(buf));
          JNOTE("ptr==STDIN_FILENO,after Util::readAll");
          JASSERT(ret != -1) (JASSERT_ERRNO);
          if (ret > 0) {
            JNOTE("ret>0.next is handleUserCommand");
          ...
          JTRACE("client of onData()")(events[n].data.ptr);
