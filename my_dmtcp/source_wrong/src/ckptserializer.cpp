...
int dmtcp::CkptSerializer::openCkptFileToRead(const dmtcp::string& path)
{
  ...
  JTRACE("check: file path")(path.c_str());
  struct timeval t0;
  struct timeval t1;
  //sawada original:end
  ...
  JTRACE("check: file path")(path.c_str())(fd)(len);
  gettimeofday(&t0, NULL);//sawada add
  JASSERT(_real_read(fd, buf, len) == len)(path) .Text("_real_read() failed");
  gettimeofday(&t1, NULL);//sawada add
  if (strncmp(buf, DMTCP_FILE_HEADER, len) == 0) {
    JTRACE("opened checkpoint file [uncompressed]")(path); // <-解凍済のファイル
    //sawada original:start
    if (t1.tv_usec < t0.tv_usec) {
      JTRACE("time1 reading ckpt")(t1.tv_sec - t0.tv_sec - 1)(1000000 + t1.tv_usec - t0.tv_usec);
    }else{
      JTRACE("time2 reading ckpt")(t1.tv_sec - t0.tv_sec)(t1.tv_usec - t0.tv_usec);
    }
    //sawada original:end
  } else {
    _real_close(fd);
