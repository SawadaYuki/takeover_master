...
//sawada 
int balanceproc20151118 = 0;
int numofProcess= 0;
std::vector<uint64_t> RecvHostIds;
std::string sProcessLocation;// s(PID1+PID2+ ... /IP1+IP2+... /IsParent + IsParent +...
//sawada



using namespace dmtcp;
//extern uint32_t  _isRootOfProcessTreeTmp;
...
static uint32_t getCkptInterval()
{
  ...
  //JTRACE("getenv ENV_VAR_CKPT_INTR")(interval);
                                                      ...
                                                      int *isRunning)
{
  ...
  //receive REPLY
  reply.poison();
  JTRACE("Before _coordinatorSocket >> reply")(c)(numPeers)(isRunning);//*num~のようにするとセグメントエラー
  // coordinatorからmsg受信
  _coordinatorSocket >> reply; // <-- ここで止っている
  JTRACE("After _coordinatorSocket >> reply"); // <-- 表示されない
  reply.assertValid();
  JTRACE("After reply.assertValid()")(reply.type);
                                                   ...
                                                   void **extraData)
{
  ...
  msg->poison();
  _coordinatorSocket >> (*msg);
  JTRACE("After msg->poison")(msg->type)(msg->state)(msg->extraBytes);

  if (extraData != NULL) {
    JTRACE("extraData != NULL");
    msg->assertValid();
    JASSERT(msg->extraBytes > 0);
                                                ...
                                                mode)
{
  ...
  if (fork() == 0) {
    ...
    JTRACE("After fork.So fork() ==0 ,this is child process")(coordinator.c_str());
                                                      ...
                                                      UniquePid *compId)
{
  ...
  //JTRACE("Before _coordinatorSocket << msg")(_isRootOfProcessTreeTmp)(msg.realPid);
  _coordinatorSocket << msg;
  JTRACE("After _coordinatorSocket << msg,next writeAll")(hostname)(progname)(msg.theCheckpointInterval);
  ...
  JTRACE("receive msg.type == DMT_ACCEPT ?")(msg.type)(msg.theCheckpointInterval);
  return msg;
}
                                                    ...
                                                    struct in_addr  *localIP)
{
  ...
  *compId = hello_remote.compGroup.upid();
  ...
  JTRACE("After memcpy")(inet_ntoa(hello_remote.ipAddr));
}
...
void dmtcp::CoordinatorAPI::createNewConnectionBeforeFork(string& progname)
{
  ...
  JTRACE("end createNewConnectionBeforeFork");
}
                                                    ...
                                                    struct in_addr  *localIP)
{
  ...
  if (localIP != NULL) {
    memcpy(localIP, &hello_remote.ipAddr, sizeof hello_remote.ipAddr);
    //memcpy(&queryIpAddr,&hello_remote.ipAddr, sizeof hello_remote.ipAddr);//sawada add
  }

  
  JTRACE("Coordinator handshake RECEIVED!!!!!")(inet_ntoa(hello_remote.ipAddr))/*(inet_ntoa(queryIpAddr))*/;
}
						       ...
						       int sync)
{
  ...
  //JTRACE("next,sock << msg");
  sock << msg;//jsocket::writeAll
  //JTRACE("next,sock.writeAll(key,key_len)");
  sock.writeAll((const char *)key, key_len);//jsocket::writeAll
  //JTRACE("next,sock.writeAll(val,val_len)");
  sock.writeAll((const char *)val, val_len);//jsocket::writeAll
  if (sync) {
    msg.poison();
    sock >> msg;
                                                  ...
                                                  uint32_t *val_len,const void *myipAddr)
{
  ...
  //memset(&msg.ipAddr, 0, sizeof myipAddr);//<- 意味なし sawada
  //void* aaa;
  memcpy((void*)&msg.ipAddr,myipAddr,sizeof myipAddr);//sawada
  
  JTRACE("sendQueryToCoordinator top")(val)(key)(inet_ntoa(msg.ipAddr));//sawada
  ...
  //dmtcp_get_local_ip_addr(&msg.ipAddr); //<-undefined reference to `dmtcp_get_local_ip_addr'
  

  sock << msg;
  JTRACE("sendQueryToCoordinator:write key")(key)(inet_ntoa(msg.ipAddr));//(msg.ipAddr);
  sock.writeAll((const char *)key, key_len);

  msg.poison();
  sock >> msg;
  msg.assertValid();
  //JTRACE("Got reply msg")(msg.type);
  ...
  *val_len = msg.valLen;
  ...
  JTRACE("get remote msg")(msg.numPeers);
  
  numofProcess = msg.numPeers;//sawada add:coordinatorからリスタート用に復元したプロセスの数を受け取る
  ...
  if(strcmp(msg.nsid,"b") == 0){
    balanceproc20151118 = 1; //負荷分散したプロセスあり
  ...
  if (*val_len > 0) {
    ...
    JTRACE("After write val")(ip)(port);
  }
