...
void mtcp_writememoryareas(int fd)
{
  ...
  int mapsfd = _real_open("/proc/self/maps", O_RDONLY); //→dmtcpworker.cpp:139
  while (Util::readProcMapsLine(mapsfd, &area)) {
    ...
    area.filesize = 0;
    if (area.name[0] != '\0') {
      int ffd = _real_open(area.name, O_RDONLY, 0);//do
      if (ffd != -1) {
	/*	lseek ()関数は、ファイルディスクリプタ (descriptor) fildes に対応するオープンされたファイルのオフセットを、 SEEK_ENDに基づき offset 引き数(0)の位置へ以下のように変更
	 */
        area.filesize = lseek(ffd, 0, SEEK_END);
        if (area.filesize == -1)
          area.filesize = 0;
      }
      _real_close(ffd); //ここまでdebug
    }
