...
class RestoreTarget
{
  public:
    ...
    {
      JTRACE("called createDependentChildProcess():next is fork");
      pid_t pid = fork();
      JASSERT(pid != -1);
      if (pid != 0) {
	JTRACE("I am parent proc.chicl procid = pid")(pid);
        return;
      }
      JTRACE("pid = 0:I am child proc,next is createProcess();")(pid);
      createProcess();
    }
	...
	//  JTRACE("check2")(t->upid())(_pInfo.isChild(t->upid()))(t->_pInfo.sid())(_pInfo.pid());
	  ...
	  JTRACE("next: t->createDependentChildProcess()");
          t->createDependentChildProcess();
        }
      }
...
static void runMtcpRestart(int is32bitElf, int fd, dmtcp::ProcessInfo *pInfo)
{
  ...
  JTRACE("Before execve")(newArgs[0])(environ);
